...
let starCount = 0;
    ...
    document.querySelectorAll('.icon-delete').forEach(icon => {
      icon.addEventListener('click', function() {
        ...
        // if (confirm(`Are you sure you want to delete pet? This action cannot be undone.`)) {
          fetch(`http://deti-tqs-13.ua.pt:8080/api/client/pet/delete/${petId}`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${jwtToken}`
  ...
  function saveUpdatedPetInfo(updatedPetInfo) {
    ...
    fetch(`http://deti-tqs-13.ua.pt:8080/api/client/pet/update/${updatedPetInfo.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${jwtToken}`
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('http://deti-tqs-13.ua.pt:8080/api/client/user/update', {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${jwtToken}`
